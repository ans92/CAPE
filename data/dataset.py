...
class ImageLoader:
    def __init__(self, root):
        self.root_dir = root
#        print('### ImageLoader Class __init__ function executed ###')
    def __call__(self, img):
        img = Image.open(ospj(self.root_dir,img)).convert('RGB') #We don't want alpha
#        img = imread(ospj(self.root_dir,img))
#        print('### ImageLoader Class __call__ function executed ###')
        return img
...
#    print('### dataset.py dataset_transform function executed ###')
    return transform
...
def filter_data(all_data, pairs_gt, topk = 5):
    ...
    print('Samples ', len(data), ' out of ', len(all_data)); print('### dataset.py filter_data function execute ###')
...
#        print(self.attrs)
        ...
        if not self.update_features:
            feat_file = ospj('/home/ans',root, model+'_featurers.t7')
    ...
    def parse_split(self):
        ...
        def parse_pairs(pair_list):
            ...
            with open(pair_list, 'r') as f:
                ...
                #pairs_temp = f.readlines()
                #pairs = preprocess(pairs_temp);print(pairs)
        ...
        tr_attrs, tr_objs, tr_pairs = parse_pairs(
            ospj('/home/ans',self.root, self.split, 'train_pairs.txt')
        )
        vl_attrs, vl_objs, vl_pairs = parse_pairs(
            ospj('/home/ans',self.root, self.split, 'val_pairs.txt')
        )
        ts_attrs, ts_objs, ts_pairs = parse_pairs(
            ospj('/home/ans',self.root, self.split, 'test_pairs.txt')
        )
        print((len(tr_attrs)),'\n',(len(vl_attrs)), (len(ts_attrs)))
        #now we compose all objs, attrs and pairs
        all_attrs, all_objs = sorted(
            list(set(tr_attrs + vl_attrs + ts_attrs))), sorted(
                list(set(tr_objs + vl_objs + ts_objs)))
        all_pairs = sorted(list(set(tr_pairs + vl_pairs + ts_pairs)))
#        print('### parse_split function executed ###')
    ...
    def get_split_info(self):
        ...
        '''
        data = torch.load(ospj('/home/ans',self.root, 'metadata_{}.t7'.format(self.split)))
        # print('data:',data)
