...
class MLP(nn.Module):
    ...
    def __init__(self, inp_dim, out_dim, num_layers = 1, relu = True, bias = True, dropout = False, norm = False, layers = []):
        ...
        # print('### common.py MLP class __init__ func end ###')
    def forward(self, x):
        # print('### common.py MLP class forward func end ###')
        return self.mod(x)
...
class Reshape(nn.Module):
    def __init__(self, *args):
        ...
        self.shape = args
        print('### common.py Reshape class __init__ func end ###')
    def forward(self, x):
        print('### common.py Reshape class forward func end ###')
...
def calculate_margines(domain_embedding, gt, margin_range = 5):
    ...
    margin /= max_margin
    margin *= margin_range
    print('### common.py calculate_margines func end ###')
    return margin
...
def l2_all_batched(image_embedding, domain_embedding):
    ...
    print('### common.py l2_all_batched func end ###')
    return l2_loss
...
def same_domain_triplet_loss(image_embedding, trip_images, gt, hard_k = None, margin = 2):
    ...
    print('### common.py same_domain_triplet_loss func end ###')
    return triplet_loss
...
def cross_domain_triplet_loss(image_embedding, domain_embedding, gt, hard_k = None, margin = 2):
    ...
    # Calculating the final loss
    triplet_loss = triplet_loss.sum() / (num_positive_triplets + 1e-16)
    print('### common.py cross_domain_triplet_loss func end ###')
    return triplet_loss
...
def same_domain_triplet_loss_old(image_embedding, positive_anchor, negative_anchor, margin = 2):
    ...
    triplet_loss = triplet_loss.sum() / (num_positive_triplets + 1e-16)
    print('### common.py same_domain_triplet_loss_old func end ###')
    return triplet_loss
...
def pairwise_distances(x, y=None):
    ...
    print('### common.py pairwise_distances func end ###')
...
class Evaluator:
    ...
    def __init__(self, dset, model):
        ...
        self.dset = dset
        print('### common.py Evaluator class __init__ func start ###')
        ...
        self.pair2id = dset.pair2idx
        self.attr2id = dset.attr2idx
        self.obj2id = dset.obj2idx
        
        self.train_ = dset.train_pairs
        ...
        # print('### common.py Evaluator class __init__ func ends ###')
    ...
    def generate_predictions(self, scores, obj_truth, bias = 0.0, topk = 5): # (Batch, #pairs)
        ...
        def get_pred_from_scores(_scores, topk):
            ...
            # print('self.pairs',self.pairs);print('pair_pred:',pair_pred)
            attr_pred, obj_pred = self.pairs[pair_pred][:, 0].view(-1, topk), \
                self.pairs[pair_pred][:, 1].view(-1, topk)
            
            # print('### common.py Evaluator class get_pred_from_scores func ends ###')
        ...
        # print('### common.py Evaluator class generate_predictions func ends ###');
        return results
    ...
    def score_clf_model(self, scores, obj_truth, topk = 5):
        ...
        # print('### common.py Evaluator class score_clf_model func ends ###')
        return results
    ...
    def score_manifold_model(self, scores, obj_truth, bias = 0.0, topk = 5):
        ...
        # print('### common.py Evaluator class score_manifold_model func ends ###')
        return results
    ...
    def score_fast_model(self, scores, obj_truth, bias = 0.0, topk = 5):
        ...
        # print('### common.py Evaluator class score_fast_model func ends ###')
        return results
    ...
    def evaluate_predictions(self, predictions, attr_truth, obj_truth, pair_truth, allpred, topk = 1):
        ...
        # ori_attr, ori_obj, ori_pair, seen_unseen = [], [], [], []
        # for _ in pair_truth:
                
        #     for pair, _id in self.pair2id.items():
        #         if _ == _id:
        #             ori_pair.append(pair)
        #             if pair in self.train_:
        #                 seen_unseen.append('seen')
        #             else:
        #                 seen_unseen.append('unseen')
                        
        # for _ in attr_truth:
        #     for at, _id in self.attr2id.items():
        #         if _ == _id:
        #             ori_attr.append(at)
        # for _ in obj_truth:
        #     for ob, _id in self.obj2id.items():
        #         if _ == _id:
        #             ori_obj.append(ob)
        #print(ori_pair);print(ori_attr);print(ori_obj)
        # results__ = predictions['closed']
        # pred_attr, pred_obj = [],[]
        # for _ in results__[0]:
        #     for at, _id in self.attr2id.items():
        #         if _ == _id:
        #             pred_attr.append(at)
                    
        # for _ in results__[1]:
        #     for ob, _id in self.obj2id.items():
        #         if _ == _id:
        #             pred_obj.append(ob)
        ...
        def _process(_scores):
            ...
            # print('### common.py Evaluator class _process function ends ###')
            return attr_match, obj_match, match, seen_match, unseen_match
            

        def _add_to_dict(_scores, type_name, stats):
            base = ['_attr_match', '_obj_match', '_match', '_seen_match', '_unseen_match']
            # print('### common.py Evaluator class _add_to_dict function ends ###')
        ...
        # print('scores',scores.shape, scores);print('unseen_ind',unseen_ind);print('correct_scores',correct_scores)
        ...
        # print('### common.py Evaluator class evaluate_predictions func ends ###')
        # return stats, ori_pair, pred_attr, pred_obj, seen_unseen
        
        st['best_unseen'] = np.max(unseen_accuracy)
        st['best_seen'] = np.max(seen_accuracy)
        st['AUC'] = area
        st['best_hm'] = max_hm
        return st