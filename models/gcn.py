...
def normt_spm(mx, method='in'):
    print('### gcn.py normt_spm function starts ###')
    if method == 'in':
...
def spm_to_tensor(sparse_mx):
    ...
    print('### gcn.py spm_to_tensor function executed ###')
...
class GraphConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout=False, relu=True):
        ...
        if relu:
            ...
            self.relu = None
        print('### gcn.py GraphConv class __init__ func executed ###')
    def forward(self, inputs, adj):
        print('GraphConv input',inputs)
        ...
        print('### gcn.py GraphConv class forward func ends ###')
        print('GraphConv output',outputs)
        return outputs
...
class GCN(nn.Module):
    ...
    def __init__(self, adj, in_channels, out_channels, hidden_layers):
        super().__init__()
        print('### gcn.py GCN class __init__ starts ###')
        ...
        layers.append(conv)
        print('### gcn.py GCN class __init__ ends ###')
        self.layers = layers
    ...
    def forward(self, x):
        if self.training:
            ...
            for conv in self.layers:
                x = conv(x, self.adj)
        print('### gcn.py GCN class forward func ends ###')
...
class GraphConvolution(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout=False, relu=True, residual=False, variant=False):
        ...
        print('### gcn.py GraphConvolution class __init__ func ends ###')
    def reset_parameters(self):
        stdv = 1. / math.sqrt(self.out_features)
        self.weight.data.uniform_(-stdv, stdv)
        print('### gcn.py GraphConvolution class reset_parameters func ends ###')
    def forward(self, input, adj , h0 , lamda, alpha, l):
        ...
        print('### gcn.py GraphConvolution class forward func ends ###')
        return output
...
class GCNII(nn.Module):
    def __init__(self, adj, in_channels , out_channels, hidden_dim, hidden_layers, lamda, alpha, variant, dropout = True):
        ...
        print('### gcn.py GCNII class __init__ func ends ###')
    def forward(self, x):
        _layers = []
        ...
        print('### gcn.py GCNII class forward func ends ###')
        return layer_inner