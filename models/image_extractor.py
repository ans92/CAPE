...
class ResNet18_conv(ResNet):
    ...
    print('### image_extractor.py ResNet18_conv class __init__ function exec ###')    
    def forward(self, x):
        # change forward here
        x = self.conv1(x)
        x = self.bn1(x)
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        print('### image_extractor.py ResNet18_conv class forward function exec ###')
        return x
...
def get_image_extractor(arch = 'resnet18', pretrained = True, feature_dim = None, checkpoint = ''):
    ...
    print('### image_extractor.py get_image_extractor function ends ###')
    return model
